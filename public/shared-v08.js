window.Classroom=(function(){
  ...
  function aggregate(key){const out={};answers(key).forEach(row=>{if(typeof row.response==='string')out[row.response]=(out[row.response]||0)+1});return out}
  ...
  return{get session(){return session},get snapshot(){return snapshot},device,esc,loadSession,api,refresh,current,elapsed,answers,aggregate,changedMinds,participation,timerText,state,answer,join,heartbeat};
})();
